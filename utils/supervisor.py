...
def get_arch(args):
    ...
    if args.dataset == 'imagenet':
        from utils.SRA.imagenet.resnet import resnet50, resnet34, resnet18, resnet101
        from utils.SRA.imagenet.vgg import vgg13_bn
        return resnet18

    if args.defense == 'badnet':
        from utils.SRA.imagenet.resnet import resnet34, resnet50
        return resnet50
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5, 7],
    ...
    # --------------------- set up normalizer and denormlizer ------------------------
    ...
    # ---------------------------- set up poison transformer for dataset + trigger ------------------------
    ...
    if poison_type in ['basic', 'badnet', 'blend', 'clean_label', 'refool',
        ...
        if poison_type == 'basic':
            ...
            poison_transform = blend.poison_transform(img_size=img_size, trigger=trigger,
                                                      target_class=target_class, alpha=alpha, dataset_name=dataset_name)
